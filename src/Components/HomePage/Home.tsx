...
const mapStateToProps = ({ tours, bookings }: AppProps): { tours: TourType[] } => ({
  tours: tours.tours.filter(tour => bookings.bookings.filter(booking => booking.id === tour.id).length === 0)
});
